...
for name, data in downsampled_dfs.items():
    df_processed = process_behavior(data, cues = ['CUE LIGHT ACTIVE'])#, command_df=command_df)
    processed_dfs[name] = df_processed
print('Behavior Data Processed!')

#Analyze task strategies for all mice during presentations
strat_data={}
for name, data in processed_dfs.items():
    strat_list = task_strat(data)
    strat_data[name] = strat_list
print('Task Strategy analyzed!')
...
export_path = input("Please enter file path for task strategy data:")
filename = input("Please enter name of file to be exported for task strategy data:")

export_dir = Path(export_path)
export_dir.mkdir(parents=True, exist_ok=True)
cs_pl_task_strat_df.to_csv(export_dir/f"{filename}.csv", index=True)
print('Task strategy data exported!')
